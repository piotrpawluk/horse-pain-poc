...
**Reading — MLLM threshold bifurcation, not "non-collapsed configuration aligns best."** The table reveals two distinct MLLM behavior modes, asymmetrically distributed across the test matrix. **Six of seven configurations** (every Prompt B / C variant on either Gemini, both Gemini 3.1 Pro Preview prompts, and both Qwen v2 prompts) sit in **refusal-collapse mode**: bg-rate ≥ 94 %, lose 3–8 pp moving from RME to audit labels because the audit identifies more action clips and the collapsed models miss them. **One configuration** — Gemini 2.5 Pro + Prompt A — sits in **over-detection mode**: bg-rate 33 %, gains 8.3 pp under audit-permissive labels because its "any visible motion" threshold approximately matches the audit's threshold. **Neither mode matches EquiFACS** (the published RME labels): collapsed mode operates above the EquiFACS threshold, over-detection mode operates at or below it. The MLLM threshold is therefore prompt-conditional, not capability-bound — the same model family produces opposite failure modes depending on prompt framing. See [Lesson 17](../docs/lessons_learned.md) for the centerpiece treatment, the asymmetry framing (1 of 7 configurations non-collapsed), and the implications (iter-6.5 inverse-LOSO retroactively explained as calibration finding; MLLM-as-naive-observer-screener track opens for clinical pre-screening; Lesson 14 needs refinement).

**Guardrail — 0.694 is calibration, not capability.** Gemini 2.5 + Prompt A's 0.694 agreement against Piotr-permissive labels is not "Gemini 2.5 + A is the right classifier." It is a model that disagrees with EquiFACS in the direction the audit also disagrees with EquiFACS, and a 33 % bg-rate means the model over-calls action on roughly 2/3 of clips. **No MLLM achieves the §4 "materially cleaner" gate** (≥ 0.70 agreement AND < 80 % bg-rate) on either label set. Gemini 2.5 + A is the closest at permissive 0.694, still below the 0.70 threshold, and its low bg-rate disqualifies it from the gate independently. The framing is "different threshold than EquiFACS, closer to audit than EquiFACS coders" — not "useful classifier." The MLLM-as-classifier track remains closed within the scope tested. **All numbers above are subject to the single-observer caveat in Lesson 17 — pending Step 3 within-observer consistency check.**
