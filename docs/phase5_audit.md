...
**Robust to label-tightening, not to behavior-mismatch.** Set C (tightened-rubric relabel, `eye_relabel_unmasked.txt`) gives identical aggregate counts (9/5/+4/1.80) — and crucially, Set B and Set C agree on **13 of 14 directional clips** (all 9 V3_NEWLY_RECOVERED minus 1 swap, all 5 V3_NEWLY_LOST minus 1 swap). The single swap is the same pair: `action_S5.mp4_5_` is V3_NEWLY_LOST under Set B but V3_NEWLY_RECOVERED under Set C, and `background_S10.mp4_3_` does the inverse. This swap is exactly the Sensitivity 1 structural-cancellation mechanism — Set C flips one clip's label (Set C calls `action_S5_5_` BACKGROUND because "head moving up, eyes still"; Set C calls `bg_S10_3_` BACKGROUND because "slight muscle tension increase above the orbit" was reclassified by tightened rubric as below threshold), v3's score on each doesn't change, and the directional category flips with the label. Aggregate counts preserved by single-swap, not by random coincidence.

Set A (RME filename taxonomy) collapses to net zero (7/7/+0/1.00) and shares only 3/5 LOST + 5/9 RECOV identity with Set B. **Set A is a clean negative control**: when scored against publisher-filename labels (which neither Phase 3 nor Phase 5 trained on, and which target ear motion not eye motion), the prediction shift between v1 and v3 is symmetric. This confirms the +0.117 AUC delta and the recovered/lost asymmetry come from the labeled-target alignment with eye-region content, not from any artifact tied to the filename taxonomy or to general "any cropping change shifts predictions."
