...
| 2026-05-09 | `932734862b5b5ca8479fd5cb9fb61e593c99ed49e21f905208869a82af8193e1` | 23946 | `docs/phase5_audit.md` | Phase 5 audit doc — Middle-band decision + 6 interpretive subtleties locked BEFORE per-fold S8 inspection: (1) DeLong-vs-permutation gap explained as different-nulls finding, not test-calibration noise; (2) DeLong gap is itself a finding — v3 shifts which clips are well-classified rather than uniformly improving (per-clip Phase 3 vs Phase 5 diff named as Phase 6 diagnostic); (3) Sensitivity 1 caveat: rubric-clean vs neutral-on-low-confidence-clips mechanisms unresolved at n=34; (4) margin curve sub-noise alternating-peaks pattern documented as "two distinct informative regimes" hypothesis for Phase 6 replication; (5) Lesson 11 anchor explicit — top of pre-locked realistic LOSO band (0.70–0.80, ≥0.85 unrealistic); (6) factor-(d) framing softened — same suppression direction across v2 + v3 indicates clip-level property orthogonal to crop, not crop-fixable. Plus locked S8/catchlight pre-result interpretation. Earlier hashes `355ceff9...`, `dd36072b...` superseded. |
